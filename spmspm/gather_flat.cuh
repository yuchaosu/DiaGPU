...
/* ============================================================
 * BALANCED variant — work-proportional tiles (the "imbalance" lever).
 *
 * gather_flat_kernel above tiles every C-diagonal into equal POS-position
 * tiles, so every block covers the same #positions but a DIFFERENT #pairs:
 * a C-diagonal near offset 0 has ~2*|B| pairs, a far one has a handful, so
 * a block's inner-loop work (positions * np) varies by up to ~|B|x across
 * blocks. GPU dynamic scheduling hides most of that, but the heaviest blocks
 * set the tail. Here the HOST sizes each tile's position span INVERSELY to
 * that C-diagonal's pair count, so every block does ~equal work (npos*np ~=
 * const). Still atomic-free: each block owns a disjoint position range of one
 * C-diagonal and writes Cv directly (no split-K, no reduction pass).
 *
 * A BalTile is (k, p0, npos): C-diagonal k, positions [p0, p0+npos).
 * ============================================================ */
struct BalTile { int k, p0, npos; };

template <int MAXP>
__global__ void gather_flat_bal_kernel(
    const float*   __restrict__ Av,
    const float*   __restrict__ Bv,
    const BalTile* __restrict__ tiles,
    const int*     __restrict__ pairPtr,
    const GPair*   __restrict__ pairs,
    float*         __restrict__ Cv,
    const size_t*  __restrict__ C_starts,
    const int*     __restrict__ C_len)
{
    const BalTile t = tiles[blockIdx.x];
    const int k  = t.k;
    const int p0 = pairPtr[k];
    const int np = pairPtr[k + 1] - p0;
    ...
    const size_t cbase = C_starts[k];
    int hi = t.p0 + t.npos;
    if (hi > lenC) hi = lenC;                       // clamp to this C-diagonal
    for (int p = t.p0 + threadIdx.x; p < hi; p += blockDim.x) {
        float acc = 0.0f;
        for (int q = 0; q < np; ++q) {
            const int pa = p + sAsh[q];
            const int pb = p + sBsh[q];
...
/* Build work-proportional tiles from an ALREADY-BUILT pairPtr (so it composes
 * with the existing plan build — only the tile list changes).
 *   pairPtr : Cn+1 prefix sums of pairs per C-diagonal (np[k]=pairPtr[k+1]-[k])
 *   C_len   : per-C-diagonal length (positions)
 *   THREADS : block width (positions covered per inner sweep = THREADS)
 *   POS_MAX : cap on positions/tile so a thin (np small) C-diagonal still yields
 *             enough blocks for occupancy instead of one giant block.
 * npos[k] is chosen so npos[k]*np[k] ~= THREADS*maxNp (the heaviest C-diagonal
 * gets exactly THREADS positions/block; lighter ones proportionally more, capped
 * at POS_MAX). Result: block work is ~uniform; total work is unchanged. */
inline std::vector<BalTile> build_bal_tiles(
    const std::vector<int>& pairPtr,
    const std::vector<int>& C_len,
    int THREADS, int POS_MAX)
{
    const int Cn = (int)C_len.size();
    int maxNp = 1;
    for (int k = 0; k < Cn; ++k) maxNp = std::max(maxNp, pairPtr[k+1]-pairPtr[k]);
    const long W = (long)THREADS * maxNp;                 // target work/block
    std::vector<BalTile> tiles;
    for (int k = 0; k < Cn; ++k) {
        const int np = std::max(1, pairPtr[k+1]-pairPtr[k]);
        int npos = (int)(W / np);                          // positions/block
        npos = std::max(THREADS, std::min(npos, POS_MAX)); // floor THREADS, cap POS_MAX
        for (int p = 0; p < C_len[k]; p += npos)
            tiles.push_back(BalTile{k, p, npos});
    }
    return tiles;
}
