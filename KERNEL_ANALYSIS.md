...
7. **Smem hash table to reduce atomicAdd contention (A-centric design)**: An alternative A-centric SpMM design processes each A nonzero once — each thread owns one A element and iterates all matching B diagonals to produce partial products for C. The problem: multiple A elements on different diagonals can contribute to the same output C element `C[r][c]`, so threads must atomicAdd to global C. On heavily overlapping diagonals this produces severe atomic contention. Smem fix: maintain a thread-block-local hash table in smem keyed by `(row, col)` of the target C element (or equivalently by `(diagonal, position)` within C). Each thread checks whether its target C element is already in the hash table; if so it accumulates locally (no atomic needed), otherwise it inserts a new entry. At the end of the block, each unique entry issues a single `atomicAdd` to global C. This reduces global atomic traffic from O(nnz_A × avg_B_contributing_diags) to O(unique_C_elements_per_block). Cost: smem hash table size (e.g., 512–1024 slots of `{int key; float val}`) plus collision-handling logic. This is distinct from the C-centric kernel's `smem_B_lookup` array, which is a dense positional index with no atomics involved.
